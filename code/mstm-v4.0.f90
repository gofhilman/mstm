...
!               if(.not.random_orientation) s11scale=pi*s11scale
            ...
            if(random_orientation) then
               s11scale=1.d0/sm_coef(1,1,0)
               sm_coef=sm_coef*s11scale
               write(outunit,'('' random orientation scattering matrix expansion coefficients'')')
               write(outunit,'(''    n       11          12          13          14          &
               22          23          24          33          34          44'')')
               do n=0,2*t_matrix_order
                  write(outunit,'(i5,10es12.4)') n,sm_coef(1,1,n),sm_coef(1,2,n),sm_coef(1,3,n),sm_coef(1,4,n), &
                     sm_coef(2,2,n),sm_coef(2,3,n),sm_coef(2,4,n),sm_coef(3,3,n),sm_coef(3,4,n),sm_coef(4,4,n)
               enddo
            endif
         endif
